...
def build_article_detail_response(db: Database, *, article_id: int) -> tuple[int, Any]:
...
def register_query_routes(server: Any) -> None:
    ...
    server._routes[("GET", "/api/recent-articles")] = lambda query: get_recent_articles_handler(
        db=server.db,
    ...
    server._routes[("GET", "/api/calibration/status")] = lambda _query: get_calibration_status_handler()
    server._routes[("GET", "/api/article-detail")] = lambda query: build_article_detail_response(
